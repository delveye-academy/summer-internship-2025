...
public class MultithreadedNumberGameApp {
	...
	private static int getThreadCountFromUser(Scanner scanner) {
		...
		System.out.println(
				"Select a number of guessing threads (between 1 and " + MAX_NUMBER_OF_GUESSING_THREADS + "): ");
		...
		while (true) {
			try {
				...
				System.out
						.println("Please only write one number between 1 and " + MAX_NUMBER_OF_GUESSING_THREADS + ": ");
	...
	private static boolean getGuessingOrderFromUser(Scanner scanner) {
		...
		while (true) {
			...
			if (userInputString.equals("1") || userInputString.equals("option1") || userInputString.equals("option 1"))
				...
				System.out.println("Invalid input, try again. Write '1' for option 1 or " + "'2' for option 2");
		}
