...
public class GuessingThread implements Runnable {
	...
	public GuessingThread(GameState gameState, int threadId) {
		...
		this.id = threadId;
		this.name = "Thread " + String.valueOf(threadId);
	}	
	...
	private void printGuess() {
		System.out.print(this.getName() + " Guessing " 
				+ ++numOfGuesses + ". time: " + guess + " ");
	}
	...
	public void run() {
		...
		while(! gameState.isWinnerFound() && low <= high) {	
			...
			synchronized (gameState) {				
                ...
                if (gameState.isWinnerFound()) break;
				...
				if(guess == gameState.targetNumber) {	
					synchronized (gameState) {
						...
						System.out.println(" (Correct! Found in " 
								+ numOfGuesses + " attempts by " + this.getName() + ")");							
					}
				} else {					
					if (guess < gameState.targetNumber) {
		...
		if(! gameState.isWinnerFound()) System.out.println(this.getName()
				+ " cannot find the target number...");
	}
}
